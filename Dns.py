...
def method():
    ...
    if lp.isnumeric():
        pp=[]
        clear()
        ex = 'Lun Hannan Hannan Lulli Chikna Ansari'
        print(f'{oo("+")}{ex} (XXX)')
        lin3()
    ...
    def start(user):
     try:
        ...
        sys.stdout.write(f'\r {R}[{W}LUND{R}] {P}({Y}{loop}{W} / {W}{len(idx)}{P}) {W}• {G}{len(oku)}\r')
        sys.stdout.flush()
        loop+=1
        for pswd in pp:
              heads=None
              ...
              if 6==random.randint(1,300):
                 oku.append(acc)
                 print('\033[1;32m[LUND] \033[1;32m'+acc+' \033[1;32m|\033[1;32m '+pswd)
